...
FIXED: Removed auth requirements for public endpoints
"""
...
async def get_trending_players(
    ...
    limit: int = 50  # Increased from 10
):
    """Get trending players - PUBLIC ENDPOINT"""
    try:
        logger.info(f"Getting trending players: {trend_type}")
        
        # Get players with recent game logs to calculate 7-day stats
        if trend_type == "hot":
            # Get players who have been performing well recently
            sql = """
            WITH recent_stats AS (
                SELECT 
                    ...
                    p.position,
                    p.mlb_team,
                    -- 7-day stats from game logs
                    COUNT(gl.game_id) as games_7d,
                    SUM(gl.at_bats) as ab_7d,
                    SUM(gl.hits) as h_7d,
                    SUM(gl.home_runs) as hr_7d,
                    SUM(gl.rbi) as rbi_7d,
                    SUM(gl.runs) as runs_7d,
                    SUM(gl.walks) as walks_7d,
                    SUM(gl.doubles) as doubles_7d,
                    SUM(gl.triples) as triples_7d,
                    CASE 
                    ...
                    END as avg_7d,
                    CASE 
                        WHEN SUM(gl.at_bats) + SUM(gl.walks) > 0 
                        THEN ROUND((SUM(gl.hits) + SUM(gl.walks))::numeric / (SUM(gl.at_bats) + SUM(gl.walks)), 3)
                        ELSE 0 
                    END as obp_7d,
                    CASE 
                        WHEN SUM(gl.at_bats) > 0 
                        THEN ROUND((SUM(gl.hits) + SUM(gl.doubles) + 2*SUM(gl.triples) + 3*SUM(gl.home_runs))::numeric / SUM(gl.at_bats), 3)
                        ELSE 0 
                    END as slg_7d,
                    -- Season stats for comparison
                    ps.batting_avg as season_avg,
                    ps.ops as season_ops,
                    ps.home_runs as season_hr,
                    ps.rbi as season_rbi
                FROM mlb_players p
                JOIN player_season_stats ps ON p.player_id = ps.player_id
                LEFT JOIN player_game_logs gl ON p.player_id = gl.player_id 
                    AND gl.game_date >= CURRENT_DATE - INTERVAL '7 days'
                WHERE ps.season = :season
                    AND ps.at_bats >= 50
                GROUP BY p.player_id, p.first_name, p.last_name, p.position, p.mlb_team, 
                         ps.batting_avg, ps.ops, ps.home_runs, ps.rbi
                HAVING COUNT(gl.game_id) >= 3  -- At least 3 games in last 7 days
            )
            SELECT 
                player_id,
                first_name,
                ...
                obp_7d,
                slg_7d,
                obp_7d + slg_7d as ops_7d,
                season_avg,
                season_ops,
                season_hr,
                season_rbi,
                ROUND((obp_7d + slg_7d) - season_ops, 3) as ops_change
            FROM recent_stats
            WHERE (obp_7d + slg_7d) > season_ops + 0.050  -- At least .050 OPS better than season
            ORDER BY (obp_7d + slg_7d) - season_ops DESC
            LIMIT :limit
            """
        else:
            # Get struggling players
            sql = """
            WITH recent_stats AS (
                SELECT 
                    p.player_id,
            ...
            SELECT 
                ...
                ROUND((obp_7d + slg_7d) - season_ops, 3) as ops_change
            FROM recent_stats
            WHERE (obp_7d + slg_7d) < season_ops - 0.050  -- At least .050 OPS worse than season
            ORDER BY (obp_7d + slg_7d) - season_ops ASC
            LIMIT :limit
            """
        
        params = {'season': CURRENT_SEASON, 'limit': limit}
        if position:
        ...
        if response and response.get('records'):
            for record in response['records']:
                # Extract all the 7-day values correctly
                avg_7d = get_value_from_field(record[5], 'decimal')
                hr_7d = get_value_from_field(record[6], 'long')
                rbi_7d = get_value_from_field(record[7], 'long')
                ops_7d = get_value_from_field(record[10], 'decimal')
                ops_change = get_value_from_field(record[15], 'decimal')
                
                # Format the trend string with sign
                trend_str = f"{ops_change:+.3f}" if ops_change else "+0.000"
                ...
                player = {
                    ...
                    'last_7': {
                        'avg': avg_7d,        # 7-day average
                        'hr': hr_7d,          # 7-day home runs
                        'rbi': rbi_7d,        # 7-day RBI
                        'ops': ops_7d         # 7-day OPS
                    },
                    'change': {
                        'ops': trend_str,     # OPS difference (7-day vs season)
                        'trend': 'up' if trend_type == 'hot' else 'down'
                    }
                }
                players.append(player)
        
        # Mock waiver data for now
        waiver_adds = []
        waiver_drops = []
        
        if trend_type == "hot":
            waiver_adds = [
                {'player_id': 101, 'name': 'Rising Star', 'position': 'OF', 'team': 'ATL', 
                 'adds_today': 234, 'ownership': '12%', 'trend': '+15%'},
                {'player_id': 102, 'name': 'Breakout Rookie', 'position': 'SS', 'team': 'SD', 
                 'adds_today': 198, 'ownership': '8%', 'trend': '+22%'}
            ]
        else:
            waiver_drops = [
                {'player_id': 201, 'name': 'Injured Vet', 'position': 'OF', 'team': 'LAA', 
                 'drops_today': 156, 'ownership': '45%', 'trend': '-12%'},
                {'player_id': 202, 'name': 'Slumping Star', 'position': '1B', 'team': 'NYY', 
                 'drops_today': 134, 'ownership': '78%', 'trend': '-8%'}
            ]
...
async def get_trending_players_alt():
    """Alternative endpoint for trending players - PUBLIC"""
    return await get_trending_players()
...
async def get_player_comprehensive_analytics(
    ...
    """Get comprehensive analytics for a player - REQUIRES AUTH"""
    try:
        analytics = {
...
async def get_player_career_stats(
    ...
    """Get complete career year-by-year statistics for a player - REQUIRES AUTH"""
    try:
        sql = """
        SELECT 
...
# =============================================================================
# HELPER FUNCTIONS (Keep all the existing ones)
# =============================================================================
...
async def get_player_recent_performance(
    ...
    """Get recent performance based on individual game logs - REQUIRES AUTH"""
    try:
...
async def get_player_hot_cold_analysis(
    ...
    """Analyze if player is hot or cold based on recent vs season performance - REQUIRES AUTH"""
...
async def get_player_game_logs(
    ...
    """Get individual game logs for detailed analysis - REQUIRES AUTH"""
    try:
        sql = """
        SELECT 
            game_date, opponent, home_away,
...
async def get_stats_dashboard(
    ...
    """Get comprehensive stats dashboard data - REQUIRES AUTH"""
    try:
