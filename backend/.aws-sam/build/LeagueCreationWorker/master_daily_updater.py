# analytics/player_analytics.py
"""
Enhanced Player Analytics Module
Calculates comprehensive analytics for player profile display
"""
...
class PlayerAnalytics:
    ...
    def _calculate_hot_cold(self) -> Dict:
        ...
        try:
            ...
            # Extract values
            recent_avg = self._get_decimal_value(recent[5])
            recent_games = self._get_long_value(recent[0])
            recent_hr = self._get_long_value(recent[3])
            recent_ab = self._get_long_value(recent[1])
            
            season_avg = self._get_decimal_value(season[0])
            season_hr = self._get_long_value(season[4])
            season_ab = self._get_long_value(season[7])
            ...
            return {
                ...
                "season_stats": {
                    ...
                    "ops": self._get_decimal_value(season[3]),
                    "home_runs": season_hr
                }
            }
    ...
    def _calculate_z_scores(self) -> Dict:
        ...
        try:
            ...
            # Batting average
            if league[1] and self._get_decimal_value(league[1]) > 0:
                z_scores['batting_avg'] = round(
                    (self._get_decimal_value(player[0]) - self._get_decimal_value(league[0])) / 
                    self._get_decimal_value(league[1]), 2
                )
            
            # OPS
            if league[3] and self._get_decimal_value(league[3]) > 0:
                z_scores['ops'] = round(
                    (self._get_decimal_value(player[3]) - self._get_decimal_value(league[2])) / 
                    self._get_decimal_value(league[3]), 2
                )
            
            # Home runs
            if league[5] and self._get_decimal_value(league[5]) > 0:
                z_scores['home_runs'] = round(
                    (self._get_long_value(player[4]) - self._get_decimal_value(league[4])) / 
                    self._get_decimal_value(league[5]), 2
                )
            
            # RBI
            if league[7] and self._get_decimal_value(league[7]) > 0:
                z_scores['rbi'] = round(
                    (self._get_long_value(player[5]) - self._get_decimal_value(league[6])) / 
                    self._get_decimal_value(league[7]), 2
                )
            
            # Stolen bases
            if league[9] and self._get_decimal_value(league[9]) > 0:
                z_scores['stolen_bases'] = round(
                    (self._get_long_value(player[7]) - self._get_decimal_value(league[8])) / 
                    self._get_decimal_value(league[9]), 2
                )
    ...
    def _get_position_rankings(self) -> List[Dict]:
        ...
        try:
            ...
            position = self._get_string_value(pos_result['records'][0][0])
            ...
            if result and result.get('records'):
                for record in result['records']:
                    rankings.append({
                        'rank': self._get_long_value(record[8]),
                        'player_id': self._get_long_value(record[0]),
                        'name': self._get_string_value(record[1]),
                        'batting_avg': self._get_decimal_value(record[2]),
                        'ops': self._get_decimal_value(record[3]),
                        'home_runs': self._get_long_value(record[4]),
                        'rbi': self._get_long_value(record[5]),
                        'stolen_bases': self._get_long_value(record[6]),
                        'runs': self._get_long_value(record[7])
                    })
            ...
            if result and result.get('records'):
                for record in result['records']:
                    monthly_splits.append({
                        'month': int(self._get_decimal_value(record[0])),
                        'games': self._get_long_value(record[1]),
                        'at_bats': self._get_long_value(record[2]),
                        'hits': self._get_long_value(record[3]),
                        'home_runs': self._get_long_value(record[4]),
                        'rbi': self._get_long_value(record[5]),
                        'runs': self._get_long_value(record[6]),
                        'stolen_bases': self._get_long_value(record[7]),
                        'batting_avg': round(self._get_decimal_value(record[8]), 3)
                    })
    ...
    def _calculate_streaks(self) -> Dict:
        ...
        try:
            ...
            for i, record in enumerate(result['records']):
                hits = self._get_long_value(record[1])
                at_bats = self._get_long_value(record[2])
                walks = self._get_long_value(record[3])
                hbp = self._get_long_value(record[4])
    ...
    def _calculate_consistency(self) -> Dict:
        ...
        try:
            ...
            averages = [self._get_decimal_value(r[0]) for r in result['records']]
        ...
        try:
            ...
            if result and result.get('records') and len(result['records']) > 1:
                ...
                for i in range(len(records) - 1):
                    ...
                    yoy_data.append({
                        'year': self._get_long_value(current[0]),
                        'games': self._get_long_value(current[1]),
                        'avg_change': round(
                            self._get_decimal_value(current[2]) - self._get_decimal_value(previous[2]), 3
                        ),
                        'ops_change': round(
                            self._get_decimal_value(current[3]) - self._get_decimal_value(previous[3]), 3
                        ),
                        'hr_change': self._get_long_value(current[4]) - self._get_long_value(previous[4]),
                        'rbi_change': self._get_long_value(current[5]) - self._get_long_value(previous[5]),
                        'wins': self._get_long_value(current[7]),
                        'losses': self._get_long_value(current[8]),
                        'era_change': round(
                            self._get_decimal_value(current[9]) - self._get_decimal_value(previous[9]), 2
                        ) if current[9] else None,
                        'whip_change': round(
                            self._get_decimal_value(current[10]) - self._get_decimal_value(previous[10]), 3
    ...
    def _get_league_averages(self) -> Dict:
        ...
        try:
            ...
            if result and result.get('records'):
                ...
                return {
                    'batting_avg_avg': self._get_decimal_value(record[0]),
                    'obp_avg': self._get_decimal_value(record[1]),
                    'slg_avg': self._get_decimal_value(record[2]),
                    'ops_avg': self._get_decimal_value(record[3]),
                    'home_runs_avg': self._get_decimal_value(record[4]),
                    'rbi_avg': self._get_decimal_value(record[5]),
                    'stolen_bases_avg': self._get_decimal_value(record[6])
                }
    ...
    # Helper methods
    def _get_decimal_value(self, field) -> float:
        if not field:
            return 0.0
    ...
    def _get_long_value(self, field) -> int:
        if not field:
            return 0
        return field.get('longValue', 0)
    
    def _get_string_value(self, field) -> str:
        if not field:
            return ""
