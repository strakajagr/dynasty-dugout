...
STATUS: FIXED - Now uses league_player_id for all transactions AND CORRECTED URL PATHS
...
def get_user_team_id(league_id: str, user_id: str) -> Optional[str]:
    ...
    try:
        result = execute_sql(
            "SELECT team_id FROM league_teams WHERE user_id = :user_id::uuid",
...
async def add_player_to_team(
    ...
    try:
        ...
        player_info = execute_sql(
            "SELECT first_name, last_name FROM mlb_players WHERE player_id = :mlb_player_id::uuid",
