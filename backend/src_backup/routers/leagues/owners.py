# src/routers/leagues/owners.py (Corrected)
...
# The original int() cast could fail on startup if the env var was a float string (e.g., "72.0") or None.
try:
...
async def get_league_owners(league_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # [FIXED] Cast the user_id parameter to UUID to match the table schema.
        league_sql = """
...
async def setup_team(
    ...
    try:
        ...
        team_sql = """
            ...
            WHERE league_id = :league_id::uuid AND user_id = :user_id::uuid
        """
